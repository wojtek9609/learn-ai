  ...
  lessons: [
    {
      ...
      minutes: 9,
      terms: [
        { term: { pl: 'Eval (ewaluacja)', en: 'Eval' }, def: { pl: 'Zestaw zapisanych wejsc plus kryterium poprawnosci, uruchamiany po kazdej zmianie promptu, modelu lub retrievera. Odpowiednik <code>npm test</code> dla aplikacji LLM.', en: 'A set of saved inputs plus a pass criterion, run after every change to the prompt, model or retriever. The <code>npm test</code> of LLM apps.' } },
        { term: { pl: 'Pass rate', en: 'Pass rate' }, def: { pl: 'Odsetek przypadkow, ktore przeszly caly zestaw. Nie musi byc wysoki, musi byc <em>porownywalny</em> miedzy wersja A i B.', en: 'The percentage of cases that passed the suite. It does not have to be high, it has to be <em>comparable</em> between version A and B.' } },
        { term: { pl: 'Wariancja przebiegu', en: 'Run variance' }, def: { pl: 'Nawet przy temperature 0 wynik nie jest w pelni deterministyczny. Przy 50 przypadkach blad standardowy to okolo 4-5 punktow procentowych, wiec male roznice to szum.', en: 'Even at temperature 0 results are not fully deterministic. At 50 cases the standard error is around 4-5 percentage points, so small deltas are noise.' } },
        { term: { pl: 'Przypadek regresyjny', en: 'Regression case' }, def: { pl: 'Kazdy bug zgloszony przez uzytkownika dopisany do zestawu jako nowy przypadek, dokladnie jak test regresyjny po incydencie.', en: 'Every user-reported bug added to the suite as a new case, exactly like a regression test after an incident.' } }
      ],
      diagram: {
...

    {
      ...
      minutes: 11,
      terms: [
        { term: { pl: 'Golden set', en: 'Golden set' }, def: { pl: 'Recznie zatwierdzony zbior wejsc z oczekiwanymi odpowiedziami. Rosnie z produkcji i sluzy jako punkt odniesienia dla kazdej wersji.', en: 'A hand-curated set of inputs with expected answers. It grows from production and serves as the reference point for every version.' } },
        { term: { pl: 'Asercja deterministyczna', en: 'Deterministic assertion' }, def: { pl: 'Zwykly kod sprawdzajacy twarde warunki: poprawny schemat JSON, wywolane narzedzie, brak zakazanej frazy. Najtansza i najstabilniejsza warstwa, 60-70 procent checkow.', en: 'Plain code checking hard conditions: valid JSON schema, tool actually called, forbidden phrase absent. The cheapest and most stable layer, 60-70 percent of checks.' } },
        { term: { pl: 'Porownanie parami', en: 'Pairwise comparison' }, def: { pl: 'Zamiast oceny bezwzglednej sedzia albo czlowiek wybiera lepsza z dwoch odpowiedzi. Latwiejsze do wykonania i stabilniejsze niz skala 1-10.', en: 'Instead of an absolute score, a judge or a human picks the better of two answers. Easier to do and more stable than a 1-10 scale.' } },
        { term: { pl: 'Holdout set', en: 'Holdout set' }, def: { pl: 'Czesc przypadkow nigdy nie uzywana przy tuningu promptu. Chroni przed przeuczeniem sie pod wlasny zestaw testowy.', en: 'A slice of cases never used while tuning the prompt. It guards against overfitting to your own test set.' } },
        { term: { pl: 'A/B na ruchu', en: 'A/B on traffic' }, def: { pl: 'Ostateczny sad: dwie wersje na zywych uzytkownikach porownywane metrykami produktowymi (retencja, eskalacje, konwersja), nie samym pass rate.', en: 'The final verdict: two versions on live users compared with product metrics (retention, escalations, conversion), not just pass rate.' } }
      ],
      diagram: {
...

    {
      ...
      minutes: 12,
      terms: [
        { term: { pl: 'LLM as judge', en: 'LLM as judge' }, def: { pl: 'Uzycie modelu do oceny odpowiedzi innego modelu wedlug rubryki. Skaluje ocene tam, gdzie asercja nie wystarcza, a czlowiek jest za drogi.', en: 'Using a model to grade another model output against a rubric. It scales judgement where an assertion is not enough and a human is too expensive.' } },
        { term: { pl: 'Rubryka', en: 'Rubric' }, def: { pl: 'Jawne kryteria oceny z definicjami poziomow i przykladami. Bez rubryki sedzia ocenia styl, a nie poprawnosc.', en: 'Explicit grading criteria with level definitions and examples. Without a rubric the judge grades style, not correctness.' } },
        { term: { pl: 'Position bias', en: 'Position bias' }, def: { pl: 'Sedzia systematycznie faworyzuje odpowiedz podana jako pierwsza. Kontrola: ocena w obu kolejnosciach i usrednienie.', en: 'The judge systematically favours whichever answer comes first. Control: score both orderings and average.' } },
        { term: { pl: 'Length bias', en: 'Length bias' }, def: { pl: 'Dluzsza odpowiedz dostaje wyzsza ocene niezaleznie od tresci. Kontrola: limit dlugosci w rubryce i jawny zakaz nagradzania rozwleklosci.', en: 'Longer answers score higher regardless of content. Control: a length cap in the rubric and an explicit ban on rewarding verbosity.' } },
        { term: { pl: 'Kalibracja sedziego', en: 'Judge calibration' }, def: { pl: 'Porownanie ocen sedziego z ocenami ludzi na wspolnej probce i pomiar zgodnosci. Nieskalibrowany sedzia to metryka, ktora klamie w spojny sposob.', en: 'Comparing judge scores with human scores on a shared sample and measuring agreement. An uncalibrated judge is a metric that lies consistently.' } }
      ],
      diagram: {
...

    {
      ...
      minutes: 10,
      terms: [
        { term: { pl: 'Trace', en: 'Trace' }, def: { pl: 'Zapis calego przebiegu jednego zadania: prompt, wywolania modelu, narzedzia, retrieval, koszt i czas. Podstawowa jednostka debugowania produkcji.', en: 'The record of one full run: prompt, model calls, tools, retrieval, cost and latency. The basic unit of debugging in production.' } },
        { term: { pl: 'Span', en: 'Span' }, def: { pl: 'Pojedynczy krok wewnatrz trace (jedno wywolanie LLM, jedno narzedzie, jedno wyszukiwanie) z wlasnym czasem, wejsciem i wyjsciem.', en: 'A single step inside a trace (one LLM call, one tool, one search) with its own timing, input and output.' } },
        { term: { pl: 'Langfuse', en: 'Langfuse' }, def: { pl: 'Otwartozrodlowa platforma obserwowalnosci LLM: tracing, datasety, oceny i koszty. Alternatywa komercyjna to Braintrust.', en: 'Open-source LLM observability platform: tracing, datasets, scores and cost. The commercial alternative is Braintrust.' } },
        { term: { pl: 'OpenTelemetry GenAI', en: 'OpenTelemetry GenAI' }, def: { pl: 'Standardowe atrybuty spanow dla modeli (<code>gen_ai.request.model</code>, <code>gen_ai.usage.input_tokens</code>) pozwalajace wpiac tracing LLM w istniejacy backend obserwowalnosci.', en: 'Standard span attributes for models (<code>gen_ai.request.model</code>, <code>gen_ai.usage.input_tokens</code>) that let LLM tracing plug into an existing observability backend.' } },
        { term: { pl: 'Eksport asynchroniczny', en: 'Async trace export' }, def: { pl: 'Wysylka tracow w tle, z buforowaniem i limitem, nigdy na sciezce krytycznej odpowiedzi. Padniety kolektor nie moze wywrocic produktu.', en: 'Shipping traces in the background, buffered and capped, never on the critical response path. A dead collector must not take the product down.' } }
      ],
      diagram: {
...

    {
      ...
      minutes: 11,
      terms: [
        { term: { pl: 'Bramka ewaluacyjna', en: 'Eval gate' }, def: { pl: 'Warunek w CI, ktory blokuje merge albo release, gdy pass rate spadnie ponizej progu wzgledem baseline. Test regresji dla promptow.', en: 'A CI condition that blocks a merge or release when pass rate drops below a threshold versus baseline. The regression test for prompts.' } },
        { term: { pl: 'Smoke suite', en: 'Smoke suite' }, def: { pl: 'Maly szybki zestaw (kilkadziesiat przypadkow, minuty) odpalany przy kazdym PR. Pelny zestaw idzie przed releasem, nie przy kazdym commicie.', en: 'A small fast set (tens of cases, minutes) run on every PR. The full suite runs before release, not on every commit.' } },
        { term: { pl: 'Canary', en: 'Canary' }, def: { pl: 'Wypuszczenie nowej wersji na maly procent ruchu z porownaniem metryk jakosci i kosztu przed pelnym rolloutem.', en: 'Releasing a new version to a small slice of traffic and comparing quality and cost metrics before the full rollout.' } },
        { term: { pl: 'Drift', en: 'Drift' }, def: { pl: 'Ciche pogorszenie jakosci przy niezmienionym kodzie: zmiana rozkladu zapytan, nieaktualne dane w RAG albo aktualizacja modelu po stronie dostawcy.', en: 'Silent quality decay with unchanged code: shifting query distribution, stale RAG data or a provider-side model update.' } },
        { term: { pl: 'Pinowanie wersji modelu', en: 'Model version pinning' }, def: { pl: 'Uzywanie dokladnego identyfikatora modelu z data zamiast aliasu, plus sledzenie terminow deprecacji. Odpowiednik <code>package-lock.json</code>.', en: 'Using the exact dated model id instead of a moving alias, plus tracking deprecation dates. The <code>package-lock.json</code> of model choice.' } }
      ],
      diagram: {
