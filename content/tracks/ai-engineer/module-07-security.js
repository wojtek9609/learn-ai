  ...
  lessons: [
    {
      ...
      minutes: 12,
      terms: [
        { term: { pl: 'Prompt injection', en: 'Prompt injection' }, def: { pl: 'Atak, w którym tekst trafiający do okna kontekstowego zostaje potraktowany jako instrukcja. Model nie odróżnia danych od poleceń.', en: 'An attack where text entering the context window is treated as an instruction. The model cannot tell data from commands.' } },
        { term: { pl: 'Injection pośrednia', en: 'Indirect injection' }, def: { pl: 'Ładunek nie pochodzi od użytkownika, tylko z treści, którą agent sam pobiera: strony WWW, e-maila, dokumentu w RAG, komentarza w issue.', en: 'The payload comes not from the user but from content the agent fetches itself: a web page, an email, a RAG document, an issue comment.' } },
        { term: { pl: 'Confused deputy', en: 'Confused deputy' }, def: { pl: 'Agent działa z uprawnieniami użytkownika, ale wykonuje instrukcje osoby trzeciej. To właściwa rama myślenia o injection, nie <em>zły prompt</em>.', en: 'The agent acts with the user permissions while executing a third party instruction. That is the right frame for injection, not <em>a bad prompt</em>.' } },
        { term: { pl: 'Egress allowlist', en: 'Egress allowlist' }, def: { pl: 'Lista hostów, do których agent może w ogóle wysłać ruch. Odcina eksfiltrację danych, nawet gdy injection się powiedzie.', en: 'The list of hosts the agent may send traffic to at all. It cuts off exfiltration even when the injection succeeds.' } },
        { term: { pl: 'Dual LLM', en: 'Dual LLM' }, def: { pl: 'Wzorzec z dwoma modelami: uprzywilejowany nigdy nie widzi niezaufanej treści, a model poddany kwarantannie czyta ją bez dostępu do narzędzi.', en: 'A two-model pattern: the privileged model never sees untrusted content, and the quarantined model reads it with no tool access.' } }
      ],
      diagram: {
...

    {
      ...
      minutes: 9,
      terms: [
        { term: { pl: 'Jailbreak', en: 'Jailbreak' }, def: { pl: 'Użytkownik namawia model do złamania własnej polityki treści. Ofiarą jest polityka dostawcy, nie dane innej osoby.', en: 'The user talks the model into breaking its own content policy. The victim is the provider policy, not another person data.' } },
        { term: { pl: 'Granica zaufania', en: 'Trust boundary' }, def: { pl: 'Linia, na której dane zmieniają właściciela. Jailbreak jej nie przekracza, injection tak - i to jest kryterium, po którym je odróżniasz.', en: 'The line where data changes owner. A jailbreak does not cross it, an injection does - and that is how you tell them apart.' } },
        { term: { pl: 'Polityka treści', en: 'Content policy' }, def: { pl: 'Reguły dostawcy o tym, czego model nie powie. Egzekwowane przez trening i klasyfikatory, nie przez uprawnienia w Twoim systemie.', en: 'The provider rules about what the model will not say. Enforced by training and classifiers, not by permissions in your system.' } },
        { term: { pl: 'Model zagrożeń', en: 'Threat model' }, def: { pl: 'Odpowiedź na pytanie, kto jest atakującym: użytkownik szkodzący sobie (jailbreak) czy osoba trzecia szkodząca użytkownikowi (injection). Różne kontrole.', en: 'The answer to who the attacker is: a user harming themselves (jailbreak) or a third party harming the user (injection). Different controls follow.' } }
      ],
      diagram: {
...

    {
      ...
      minutes: 12,
      terms: [
        { term: { pl: 'OWASP Top 10 for LLM Apps', en: 'OWASP Top 10 for LLM Apps' }, def: { pl: 'Lista ryzyk używana jako szkielet modelu zagrożeń: ryzyko, gdzie występuje u nas, kontrola, jak ją weryfikujemy.', en: 'A risk list used as a threat-model skeleton: risk, where it exists here, control, how it is verified.' } },
        { term: { pl: 'Improper output handling (LLM05)', en: 'Improper output handling (LLM05)' }, def: { pl: 'Traktowanie wyjścia modelu jak zaufanego kodu: markdown wstrzykujący HTML, SQL sklejany z odpowiedzi, <code>eval</code>. Klasyczne XSS przez LLM.', en: 'Treating model output as trusted code: markdown injecting HTML, SQL built from a response, <code>eval</code>. Classic XSS via the LLM.' } },
        { term: { pl: 'Excessive agency (LLM06)', en: 'Excessive agency (LLM06)' }, def: { pl: 'Agent ma więcej narzędzi i uprawnień, niż wymaga zadanie. Kontrola: allowlista narzędzi na sesję plus bramka ludzka dla akcji nieodwracalnych.', en: 'The agent holds more tools and permissions than the task needs. Control: a per-session tool allowlist plus a human gate for irreversible actions.' } },
        { term: { pl: 'Denial of wallet (LLM10)', en: 'Denial of wallet (LLM10)' }, def: { pl: 'Nielimitowana konsumpcja: pętla agenta bez limitu kroków potrafi wygenerować czterocyfrowy rachunek w kilka godzin. Twardy limit kroków, timeout, dzienny budżet.', en: 'Unbounded consumption: an agent loop with no step cap can produce a four-figure bill in hours. Hard step limit, timeout, daily budget.' } },
        { term: { pl: 'Izolacja tenantów w wektorach (LLM08)', en: 'Vector tenant isolation (LLM08)' }, def: { pl: 'Filtrowanie po metadanych w Qdrant czy pgvector <em>nie jest</em> izolacją. Bezpieczniej: osobne kolekcje albo Row Level Security w Postgresie.', en: 'Metadata filtering in Qdrant or pgvector is <em>not</em> isolation. Safer: separate collections or Row Level Security in Postgres.' } }
      ],
      diagram: {
...

    {
      ...
      minutes: 11,
      terms: [
        { term: { pl: 'PII', en: 'PII' }, def: { pl: 'Dane osobowe: nazwiska, adresy, numery kart, identyfikatory. W aplikacji LLM lądują w logach, trace, korpusie RAG i zestawach ewaluacyjnych.', en: 'Personally identifiable information: names, addresses, card numbers, ids. In an LLM app it lands in logs, traces, the RAG corpus and eval sets.' } },
        { term: { pl: 'Redakcja i rehydratacja', en: 'Redaction and rehydration' }, def: { pl: 'Zamiana PII na tokeny przed wysyłką do modelu i podstawienie wartości z powrotem w odpowiedzi. Regex łapie karty i e-maile, nazwiska wymagają NER (np. Presidio).', en: 'Replacing PII with tokens before sending to the model and substituting values back in the answer. Regex catches cards and emails, names need NER (e.g. Presidio).' } },
        { term: { pl: 'Zero data retention', en: 'Zero data retention' }, def: { pl: 'Ustawienie lub umowa, przy której dostawca nie przechowuje treści zapytań. Domyślnie zwykle obowiązuje retencja na potrzeby abuse monitoring, rzędu 30 dni.', en: 'A setting or agreement under which the provider stores no request content. By default abuse-monitoring retention of about 30 days usually applies.' } },
        { term: { pl: 'DPA', en: 'DPA' }, def: { pl: 'Umowa powierzenia przetwarzania danych z dostawcą modelu, wraz z regionem przetwarzania (UE kontra USA) i podwykonawcami. Wymóg RODO, nie formalność.', en: 'A data processing agreement with the model provider, including the processing region (EU versus US) and subprocessors. A GDPR requirement, not paperwork.' } },
        { term: { pl: 'Wyciek promptu systemowego', en: 'System prompt leak' }, def: { pl: 'Zakładaj, że prompt systemowy wycieknie. Trzymaj tam tylko instrukcje zachowania, nigdy sekretów ani logiki, która stanowi model biznesowy.', en: 'Assume the system prompt leaks. Keep only behavioural instructions there, never secrets or logic that is your business model.' } }
      ],
      diagram: {
...

    {
      ...
      minutes: 11,
      terms: [
        { term: { pl: 'Najmniejsze uprawnienia', en: 'Least privilege' }, def: { pl: 'Agent dostaje dokładnie te narzędzia i zakresy, których wymaga bieżące zadanie, na czas jego trwania. Uprawnienia per sesja, nie per aplikacja.', en: 'The agent gets exactly the tools and scopes the current task needs, for its duration. Permissions per session, not per application.' } },
        { term: { pl: 'Sandbox wykonawczy', en: 'Execution sandbox' }, def: { pl: 'Odizolowane środowisko dla kodu i narzędzi: kontener z <code>--network=none</code>, <code>--read-only</code>, limitem CPU i pamięci oraz kasowany po przebiegu.', en: 'An isolated environment for code and tools: a container with <code>--network=none</code>, <code>--read-only</code>, CPU and memory caps, destroyed after the run.' } },
        { term: { pl: 'Blast radius', en: 'Blast radius' }, def: { pl: 'Zakres szkód możliwych, gdy agent zostanie przejęty. Projektuje się go z góry: co najgorszego może zrobić posiadany token w najgorszym scenariuszu.', en: 'The damage possible when the agent is compromised. You design it up front: what is the worst the held token can do in the worst case.' } },
        { term: { pl: 'Brama ludzka', en: 'Human gate' }, def: { pl: 'Obowiązkowa akceptacja człowieka przed akcją nieodwracalną: przelew, wysyłka maila, usunięcie rekordu, deploy na produkcję.', en: 'A mandatory human approval before an irreversible action: a transfer, sending an email, deleting a record, a production deploy.' } },
        { term: { pl: 'Audit log', en: 'Audit log' }, def: { pl: 'Niezmienialny zapis każdego wywołania narzędzia z tożsamością, argumentami i wynikiem. Bez niego nie odtworzysz incydentu ani nie udowodnisz zgodności.', en: 'An immutable record of every tool call with identity, arguments and result. Without it you cannot reconstruct an incident or prove compliance.' } }
      ],
      diagram: {
