  ...
  lessons: [
    {
      ...
      minutes: 10,
      terms: [
        { term: { pl: 'SSE', en: 'SSE' }, def: { pl: 'Server-Sent Events - jednokierunkowy strumien tekstowy po zwyklym HTTP. Domyslny transport strumieniowania odpowiedzi w Claude API i OpenAI API.', en: 'Server-Sent Events - a one-way text stream over plain HTTP. The default transport for streamed responses in the Claude API and the OpenAI API.' } },
        { term: { pl: 'EventSource kontra fetch stream', en: 'EventSource vs fetch stream' }, def: { pl: '<code>EventSource</code> jest prosty, ale tylko GET i bez naglowkow. Strumien z <code>fetch</code> plus <code>ReadableStream</code> daje POST, naglowki autoryzacji i <code>AbortController</code>.', en: '<code>EventSource</code> is simple but GET-only and header-less. A <code>fetch</code> stream plus <code>ReadableStream</code> gives you POST, auth headers and <code>AbortController</code>.' } },
        { term: { pl: 'Zdarzenia strumienia', en: 'Stream events' }, def: { pl: 'Nazwane zdarzenia opisujace przyrost odpowiedzi: <code>message_start</code>, <code>content_block_delta</code>, <code>message_stop</code>. Klient sklada z nich stan, nie tylko dokleja tekst.', en: 'Named events describing the response increment: <code>message_start</code>, <code>content_block_delta</code>, <code>message_stop</code>. The client rebuilds state from them, not just appends text.' } },
        { term: { pl: 'Wznowienie strumienia', en: 'Stream resume' }, def: { pl: 'Ponowne podlaczenie po zerwaniu polaczenia z numerem ostatniego zdarzenia (<code>Last-Event-ID</code> lub wlasne <code>seq</code>), zeby nie powtorzyc juz wyswietlonego tekstu.', en: 'Reconnecting after a dropped connection with the last event number (<code>Last-Event-ID</code> or your own <code>seq</code>) so already rendered text is not repeated.' } },
        { term: { pl: 'Buforowanie po drodze', en: 'Proxy buffering' }, def: { pl: 'Proxy, CDN albo gateway moze zebrac caly strumien i oddac go naraz. Lekarstwo: <code>Cache-Control: no-transform</code>, <code>X-Accel-Buffering: no</code> i wylaczona kompresja.', en: 'A proxy, CDN or gateway can collect the whole stream and deliver it at once. Cure: <code>Cache-Control: no-transform</code>, <code>X-Accel-Buffering: no</code> and compression off.' } }
      ],
      diagram: {
    ...
    },
    {
      ...
      minutes: 9,
      terms: [
        { term: { pl: 'Partial JSON', en: 'Partial JSON' }, def: { pl: 'Niedokonczony fragment JSON-a przychodzacy w trakcie generacji. Nie da sie go podac do <code>JSON.parse</code> bez wczesniejszego domkniecia struktury.', en: 'An unfinished JSON fragment arriving mid-generation. It cannot go into <code>JSON.parse</code> without closing the structure first.' } },
        { term: { pl: 'input_json_delta', en: 'input_json_delta' }, def: { pl: 'Zdarzenie strumienia niosace kolejny kawalek argumentow narzedzia jako <em>surowy tekst</em>, nie obiekt. Sklejasz je i parsujesz tolerancyjnie.', en: 'The stream event carrying the next slice of tool arguments as <em>raw text</em>, not an object. You concatenate and parse it leniently.' } },
        { term: { pl: 'Parser tolerancyjny', en: 'Best-effort parser' }, def: { pl: 'Parser domykajacy brakujace nawiasy i cudzyslowy, zeby zwrocic najlepszy mozliwy obiekt z niepelnego wejscia. Gotowce: <code>partial-json</code>, <code>best-effort-json-parser</code>.', en: 'A parser that closes missing brackets and quotes to return the best possible object from incomplete input. Off the shelf: <code>partial-json</code>, <code>best-effort-json-parser</code>.' } },
        { term: { pl: 'DeepPartial', en: 'DeepPartial' }, def: { pl: 'Typ opisujacy obiekt w budowie: kazde pole moze byc jeszcze <code>undefined</code>. Komponenty renderujace strumien musza byc na niego przygotowane.', en: 'The type of an object under construction: every field may still be <code>undefined</code>. Components rendering a stream must be written against it.' } },
        { term: { pl: 'Obciecie na max_tokens', en: 'max_tokens truncation' }, def: { pl: 'Strumien konczy sie z <code>stop_reason: "max_tokens"</code> i JSON nigdy sie nie domyka. Traktuj to jako blad walidacji, nie jako poprawny wynik czesciowy.', en: 'The stream ends with <code>stop_reason: "max_tokens"</code> and the JSON never closes. Treat it as a validation failure, not a valid partial result.' } }
      ],
      diagram: {
    ...
    },
    {
      ...
      minutes: 10,
      terms: [
        { term: { pl: 'Generative UI', en: 'Generative UI' }, def: { pl: 'Interfejs, w ktorym model nie zwraca tekstu, tylko wybiera komponenty i ich dane. UI powstaje z ustrukturyzowanego wyjscia w trakcie strumienia.', en: 'A UI where the model returns not text but a choice of components and their data. The interface is built from structured output as it streams.' } },
        { term: { pl: 'Rejestr komponentow', en: 'Component allowlist' }, def: { pl: 'Zamkniety slownik nazwa komponentu na implementacje po stronie klienta. Model moze wybrac tylko z listy, nigdy nie dostarcza markupu.', en: 'A closed map from component name to a client-side implementation. The model can only pick from the list and never supplies markup.' } },
        { term: { pl: 'Tool-driven UI', en: 'Tool-driven UI' }, def: { pl: 'Wywolanie narzedzia jest jednoczesnie poleceniem renderowania: <code>show_flight_options</code> zwraca dane i uruchamia odpowiedni widok.', en: 'A tool call doubles as a render instruction: <code>show_flight_options</code> returns data and mounts the matching view.' } },
        { term: { pl: 'Walidacja propsow', en: 'Prop validation' }, def: { pl: 'Kazdy zestaw propsow z modelu przechodzi przez schemat (zod) przed renderem, a URL-e maja sprawdzany protokol. Zaden <code>javascript:</code> ani surowy HTML.', en: 'Every prop set from the model passes a schema (zod) before render, and URLs get their scheme checked. No <code>javascript:</code>, no raw HTML.' } }
      ],
      diagram: {
    ...
    ,
    {
      ...
      minutes: 9,
      terms: [
        { term: { pl: 'Human-in-the-loop (HITL)', en: 'Human-in-the-loop (HITL)' }, def: { pl: 'Wzorzec, w ktorym agent zatrzymuje sie przed skutkiem ubocznym i czeka na decyzje czlowieka. Zgoda dotyczy konkretnej akcji, nie calej sesji.', en: 'A pattern where the agent pauses before a side effect and waits for a human decision. Approval covers one concrete action, not the whole session.' } },
        { term: { pl: 'Pauza na tool_use', en: 'tool_use pause' }, def: { pl: 'Model konczy ture z <code>stop_reason: "tool_use"</code>, a Ty po prostu nie odsylasz <code>tool_result</code> do momentu akceptacji. Sam protokol daje punkt wstrzymania.', en: 'The model ends its turn with <code>stop_reason: "tool_use"</code> and you simply withhold the <code>tool_result</code> until approval. The protocol itself gives you the pause point.' } },
        { term: { pl: 'Podglad diffa', en: 'Diff preview' }, def: { pl: 'Pokazanie dokladnie tego, co sie zmieni (przed i po, adresat, kwota), zamiast prosby o zgode na abstrakcyjne dzialanie.', en: 'Showing exactly what will change (before and after, recipient, amount) instead of asking approval for an abstract action.' } },
        { term: { pl: 'Progresywna autonomia', en: 'Progressive autonomy' }, def: { pl: 'Zakres akcji wykonywanych bez pytania rosnie wraz z zaufaniem: najpierw wszystko za zgoda, potem tylko odwracalne, na koncu tylko nieodwracalne.', en: 'The range of unattended actions grows with trust: first everything asks, then only irreversible ones do.' } },
        { term: { pl: 'Undo i kompensacja', en: 'Undo and compensation' }, def: { pl: 'Tanie cofniecie skutku (wersje robocze, soft delete, akcja odwracajaca) czesto bije kolejne okno potwierdzenia, bo nie meczy uzytkownika.', en: 'Cheap reversal (drafts, soft delete, a compensating action) often beats another confirmation dialog because it does not fatigue the user.' } }
      ],
      diagram: {
    ...
    },
    {
      ...
      minutes: 8,
      terms: [
        { term: { pl: 'TTFT', en: 'TTFT' }, def: { pl: 'Time to first token - czas do pojawienia sie pierwszego znaku odpowiedzi. Glowna metryka odczuwanej szybkosci, wazniejsza niz czas calkowity.', en: 'Time to first token - how long until the first character appears. The main perceived-speed metric, more important than total time.' } },
        { term: { pl: 'Tokeny na sekunde', en: 'Tokens per second' }, def: { pl: 'Tempo dalszego strumienia. Powyzej okolo 20-30 tokenow na sekunde tekst wyprzedza czytanie, wiec dalsze przyspieszanie prawie nic nie daje.', en: 'The rate of the rest of the stream. Above roughly 20-30 tokens per second text outruns reading, so further speed buys almost nothing.' } },
        { term: { pl: 'Optimistic UI', en: 'Optimistic UI' }, def: { pl: 'Natychmiastowe pokazanie stanu docelowego przed potwierdzeniem z serwera, z cofnieciem w razie bledu. Kupuje sekundy postrzeganej responsywnosci.', en: 'Showing the intended end state immediately, before the server confirms, and rolling back on error. It buys seconds of perceived responsiveness.' } },
        { term: { pl: 'Skeleton kontra spinner', en: 'Skeleton vs spinner' }, def: { pl: 'Skeleton pokazuje ksztalt nadchodzacej tresci i skraca odczuwane czekanie, spinner tylko sygnalizuje, ze cos trwa.', en: 'A skeleton shows the shape of the incoming content and shortens the felt wait, a spinner only signals that something is happening.' } },
        { term: { pl: 'Prompt caching a TTFT', en: 'Prompt caching and TTFT' }, def: { pl: 'Cache prefiksu promptu skraca faze prefill, wiec obniza TTFT nawet kilkukrotnie przy dlugim, stalym kontekscie.', en: 'Caching the prompt prefix shortens the prefill phase, cutting TTFT several times over with a long, stable context.' } }
      ],
      diagram: {
    ...
    },
    {
      ...
      minutes: 9,
      terms: [
        { term: { pl: 'Refusal', en: 'Refusal' }, def: { pl: 'Model swiadomie odmawia odpowiedzi. To poprawny wynik, nie awaria: potrzebuje wlasnego stanu UI z wyjasnieniem i sciezka dalej.', en: 'The model deliberately declines. That is a valid outcome, not a crash: it needs its own UI state with an explanation and a way forward.' } },
        { term: { pl: 'stop_reason', en: 'stop_reason' }, def: { pl: 'Pole mowiace, dlaczego generacja sie skonczyla (<code>end_turn</code>, <code>tool_use</code>, <code>max_tokens</code>). Rozroznia odpowiedz skonczona od urwanej.', en: 'The field telling you why generation ended (<code>end_turn</code>, <code>tool_use</code>, <code>max_tokens</code>). It separates a finished answer from a cut-off one.' } },
        { term: { pl: 'Fallback', en: 'Fallback' }, def: { pl: 'Zaplanowana degradacja przy bledzie: inny model, wynik wyszukiwania bez generacji albo zapisany draft. Zawsze lepsza niz pusty ekran.', en: 'Planned degradation on failure: another model, raw search results without generation, or a saved draft. Always better than a blank screen.' } },
        { term: { pl: 'Sygnalizowanie niepewnosci', en: 'Uncertainty display' }, def: { pl: 'Pokazanie zrodel, progu trafnosci retrievalu albo jawnego <em>nie wiem</em>. Buduje zaufanie szybciej niz pewny siebie ton.', en: 'Showing sources, the retrieval score threshold, or an explicit <em>I do not know</em>. It builds trust faster than a confident tone.' } },
        { term: { pl: '429 i Retry-After', en: '429 and Retry-After' }, def: { pl: 'Limit zapytan po stronie dostawcy. Klient ma czekac tyle, ile mowi naglowek <code>retry-after</code>, z backoffem, i powiedziec o tym uzytkownikowi.', en: 'A provider-side rate limit. The client should wait as long as the <code>retry-after</code> header says, with backoff, and tell the user.' } }
      ],
      diagram: {
