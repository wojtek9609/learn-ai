...
export default {
  ...
  icon: '🧩',
  title: {
    pl: 'Wzorce komponentów',
    en: 'Component patterns'
  },
  description: {
    pl: 'Sloty, provide/inject, v-model i renderless components przełożone na children, Context, kontrolowane inputy, compound components oraz Suspense i error boundaries.',
    ...
    // ------------------------------------------------------------------ 1
    {
      ...
      terms: [
        {
          ...
          def: {
            pl: 'Props, do którego kompiluje się JSX zapisany między tagami komponentu. To wartość policzona przez rodzica, a nie funkcja wołana przez dziecko jak slot we Vue.',
        ...
        },
        {
          ...
          def: {
            pl: 'Zamiennik slotów nazwanych: zwykły props przyjmujący JSX, np. <code>header</code> albo <code>footer</code>. W TypeScripcie widać w podpowiedziach, jakie dziury ma komponent.',
        ...
        },
        {
          ...
          def: {
            pl: 'Typ dla dowolnej treści renderowalnej. <code>ReactElement</code> bierz tylko wtedy, gdy naprawdę potrzebujesz dokładnie jednego elementu.',
        ...
        },
        {
          ...
          def: {
            pl: 'Reactowy odpowiednik grzebania w VNode-ach dziecka, razem z <code>React.Children.map</code>. Kruchy i odradzany w dokumentacji Reacta 19 - użyj Contextu.',
      ...
      diagram: {
        svg: '<svg viewBox="0 0 640 400" xmlns="http://www.w3.org/2000/svg" font-family="inherit">' +
          ...
          '</svg>',
        caption: {
          pl: 'Vue rozwiązuje dziury w layoucie osobnym API slotów; React przekazuje ten sam content jako zwykły props, w tym children.',
          en: 'Vue fills layout holes with a dedicated slot API; React passes the same content as an ordinary prop, children included.'
        }
      },
      levels: {
        eli5: {
          pl: '<p>Wyobraź sobie ramkę na zdjęcie. Ramka jest zawsze taka sama: ma szkło, tekturkę i zawieszkę. Ale to, co w niej wisi, wybierasz ty. Ramka nie musi wiedzieć, czy w środku jest kot, dyplom czy rysunek dziecka.</p><p>W komponentach robimy dokładnie to samo. Piszesz raz ładną ramkę, na przykład kartę z cieniem i zaokrąglonymi rogami, a to, co w niej stoi, wrzuca ktoś inny.</p><p>We Vue mówisz ramce: <strong>tu jest dziura, wsadź coś</strong>. Ta dziura nazywa się slot. W Reactie nie ma żadnej specjalnej dziury. Zawartość przekazujesz ramce jak każdą inną rzecz, na przykład jak kolor czy tytuł. Nazywa się to children, czyli po prostu dzieci.</p><p>Efekt jest identyczny. Różnica jest taka, że Vue ma osobny mechanizm na ten jeden przypadek, a React używa tego samego mechanizmu co do wszystkiego innego. Mniej pojęć do zapamiętania, więcej swobody.</p>',
        ...
        school: {
          pl: '<p>We Vue sloty to osobne API języka szablonów. Masz <code>slot</code> domyślny, sloty nazwane i scoped slots, które oddają dane z dziecka do rodzica.</p><pre><code>&lt;!-- Vue: Card.vue --&gt;\n&lt;div class="card"&gt;\n  &lt;header&gt;&lt;slot name="header" /&gt;&lt;/header&gt;\n  &lt;slot /&gt;\n&lt;/div&gt;\n\n&lt;!-- użycie --&gt;\n&lt;Card&gt;\n  &lt;template #header&gt;&lt;h3&gt;Faktura&lt;/h3&gt;&lt;/template&gt;\n  &lt;p&gt;Treść&lt;/p&gt;\n&lt;/Card&gt;</code></pre><p>W Reactie nie ma slotów, bo nie są potrzebne. JSX jest wyrażeniem, więc element można wsadzić do zmiennej, do tablicy i do propsa.</p><pre><code>// React: Card.jsx\nfunction Card({ header, children }) {\n  return (\n    &lt;div className="card"&gt;\n      &lt;header&gt;{header}&lt;/header&gt;\n      {children}\n    &lt;/div&gt;\n  );\n}\n\n&lt;Card header={&lt;h3&gt;Faktura&lt;/h3&gt;}&gt;\n  &lt;p&gt;Treść&lt;/p&gt;\n&lt;/Card&gt;</code></pre><p><strong>We Vue robiłeś slot domyślny, w Reactie robisz props <code>children</code>, bo JSX między tagami to tylko cukier składniowy na ten właśnie props.</strong> Dosłownie: <code>&lt;Card&gt;x&lt;/Card&gt;</code> kompiluje się do <code>createElement(Card, null, x)</code>.</p><p>Sloty nazwane zamieniasz na zwykłe propsy typu element: <code>header</code>, <code>footer</code>, <code>actions</code>. Konwencja w dużych design systemach jest taka, że główna treść idzie przez <code>children</code>, a wszystko dodatkowe przez nazwane propsy. Dzięki temu w TypeScripcie widzisz w podpowiedziach, jakie dziury ma komponent, czego przy slotach Vue nie dostajesz za darmo.</p><p>Scoped slots to osobna historia: skoro dziecko ma oddać dane, props musi być funkcją, a nie gotowym elementem. O tym w lekcji o render props.</p><p>Warto też zapamiętać jedną drobną różnicę w codziennej pracy: we Vue pytasz o obecność slota przez <code>$slots.header</code>, a w Reactie po prostu sprawdzasz, czy props został podany. Zero dodatkowego API, zwykły warunek w JavaScripcie.</p>',
        ...
        pro: {
          pl: '<p>Kluczowa zmiana modelu: we Vue slot jest <em>funkcja renderująca</em> tworzona przez kompilator i wywoływana przez dziecko. W Reactie <code>children</code> to <em>wartość</em>, zwykłe drzewo elementów, które rodzic policzył zanim dziecko w ogóle ruszyło.</p><pre><code>// Vue: slot jest lazy, dziecko decyduje czy go wywołać\n&lt;template&gt;\n  &lt;div v-if="open"&gt;&lt;slot /&gt;&lt;/div&gt;\n&lt;/template&gt;\n\n// React: element jest już stworzony, ale NIE zamontowany\nfunction Panel({ open, children }) {\n  return open ? &lt;div&gt;{children}&lt;/div&gt; : null;\n}</code></pre><p>Różnica jest subtelna i ważna. <code>&lt;Heavy /&gt;</code> jako children tworzy jedynie obiekt opisu (typ plus propsy) - to kosztuje nanosekundy. Ciała komponentu React nie wykona, dopóki nie znajdzie się w zwróconym drzewie. Natomiast <strong>wyrażenia</strong> w propsach liczą się od razu: <code>header={expensiveCalc()}</code> policzy się zawsze. We Vue slot by tego nie policzył. Jeśli musisz zachować leniwość, przekaż funkcję: <code>renderHeader={() =&gt; expensiveCalc()}</code>.</p><p>Druga różnica dotyczy tożsamości. Kiedy rodzic się przerenderowuje, tworzy nowe obiekty elementów dla children, więc <code>React.memo</code> na dziecku nie pomoże, jeśli children są inline. Znany trik:</p><pre><code>// children jako props: zmiana stanu Layout nie odświeża Sidebar\nfunction Layout({ children }) {\n  const [tab, setTab] = useState(0);\n  return &lt;main&gt;{children}&lt;/main&gt;;\n}\n&lt;Layout&gt;&lt;Sidebar /&gt;&lt;/Layout&gt;</code></pre><p>Element <code>&lt;Sidebar /&gt;</code> powstał w komponencie nadrzędnym; stan <code>tab</code> wewnątrz <code>Layout</code> go nie unieważnia, więc React porównuje ten sam obiekt i pomija poddrzewo. To reactowy odpowiednik tego, co we Vue dostajesz z automatu dzięki granularnej reaktywności.</p><p>Praktyka produkcyjna:</p><ul><li>Typuj jako <code>React.ReactNode</code> dla contentu i <code>React.ReactElement</code>, gdy naprawdę potrzebujesz jednego elementu.</li><li>Unikaj <code>React.Children.map</code> i <code>cloneElement</code> - to reactowy odpowiednik grzebania w VNode-ach dziecka, kruchy i oficjalnie odradzany w dokumentacji React 19. Zamiast tego użyj Context (patrz następna lekcja).</li><li>Puste children to <code>undefined</code>, nie pusta tablica. Sprawdzaj <code>children == null</code>, nie <code>children.length</code>.</li><li>Nie ma odpowiednika <code>$slots.header</code>, ale <code>props.header == null</code> daje ci to samo pytanie: czy rodzic coś podał.</li></ul><p>Na rozmowie pada: czym różni się slot od children. Dobra odpowiedź: slot jest funkcją wywoływaną przez dziecko, children są wartością obliczoną przez rodzica, i z tego wynikają różnice w leniwości oraz w tym, kiedy poddrzewo się przerenderowuje.</p>',
      ...
      quiz: [
        {
          q: {
            pl: 'Co w Reactie odpowiada domyślnemu slotowi z Vue?',
          ...
          options: [
            ...
            { pl: 'Props children, czyli JSX zapisany między tagami komponentu', en: 'The children prop, that is the JSX written between the component tags' },
            { pl: 'Dyrektywa v-slot dostępna w JSX', en: 'A v-slot directive available in JSX' },
            { pl: 'Hook useSlot', en: 'The useSlot hook' }
          ],
          correct: 1,
          explain: {
            pl: 'JSX między tagami kompiluje się do trzeciego argumentu createElement, czyli do propsa children. Żadnego osobnego API nie ma.',
        ...
        {
          q: {
            pl: 'Slot Vue jest leniwy. Co jest odpowiednikiem tej leniwości w Reactie?',
            en: 'A Vue slot is lazy. What corresponds to that laziness in React?'
          },
          options: [
            { pl: 'Element w children powstaje od razu, ale komponent nie wykona się dopóki nie trafi do zwróconego drzewa', en: 'The children element is created immediately, but the component body does not run until it lands in the returned tree' },
            { pl: 'React w ogóle nie tworzy elementów, dopóki nie są widoczne', en: 'React does not create elements at all until they are visible' },
            { pl: 'Każdy props jest obliczany leniwie', en: 'Every prop is evaluated lazily' },
            { pl: 'Leniwość działa tylko z React.lazy', en: 'Laziness only exists with React.lazy' }
          ],
          correct: 0,
          explain: {
            pl: 'Tworzenie elementu to tani obiekt opisu. Prawdziwe wywołanie funkcji komponentu następuje przy renderze. Ale wyrażenia w propsach liczą się natychmiast - tu leniwości nie ma.',
        ...
        {
          q: {
            pl: 'Jak przełożysz slot nazwany header na Reacta zgodnie z dobrą praktyką?',
            en: 'What is the idiomatic React translation of a named header slot?'
          },
          options: [
            { pl: 'Przeszukać children i wyciągnąć element o odpowiednim typie', en: 'Scan children and pull out the element with the right type' },
            { pl: 'Użyć cloneElement na pierwszym dziecku', en: 'Use cloneElement on the first child' },
            { pl: 'Dodać zwykły props header typu ReactNode', en: 'Add an ordinary header prop typed as ReactNode' },
            { pl: 'Zdefiniować globalny Context o nazwie header', en: 'Define a global Context named header' }
          ],
          correct: 2,
          explain: {
            pl: 'Nazwany props jest jawny, widoczny w typach i nie zależy od kolejności dzieci. Grzebanie w children jest kruche i odradzane.',
        ...
        {
          q: {
            pl: 'Layout ma własny stan i renderuje {children}. Dlaczego kosztowny Sidebar podany jako children nie przerenderowuje się przy zmianie tego stanu?',
          ...
          options: [
            { pl: 'Bo children są domyślnie owinięte w React.memo', en: 'Because children are wrapped in React.memo by default' },
            { pl: 'Bo element Sidebar stworzył komponent nadrzędny i to ten sam obiekt, więc React pomija poddrzewo', en: 'Because the Sidebar element was created by the grandparent and is the same object, so React skips the subtree' },
            { pl: 'Bo React porównuje wyrenderowany DOM i nie widzi różnic', en: 'Because React diffs the rendered DOM and sees no differences' },
            { pl: 'Bo przerenderuje się zawsze, to pytanie z błędem', en: 'It does re-render; the question is wrong' }
          ],
          correct: 1,
          explain: {
            pl: 'Stan Layoutu nie tworzy nowego elementu dla children. React widzi identyczną referencję i nie schodzi głębiej. To najtańszy sposób na optymalizację bez memo.',
    ...
    // ------------------------------------------------------------------ 2
    {
      ...
      terms: [
        {
          ...
          def: {
            pl: 'Mechanizm dystrybucji wartości w dół drzewa, odpowiednik <code>provide</code> i <code>inject</code>. Nie jest store: nie ma selektorów, więc zmiana wartości budzi wszystkich konsumentów.',
        ...
        {
          term: { pl: 'Stabilna wartość providera', en: 'Stable provider value' },
          def: {
            pl: 'Nowy obiekt literalny w propsie <code>value</code> renderuje wszystkich konsumentów na każdy render providera. Ratunkiem jest <code>useMemo</code> na wartości.',
        ...
        {
          term: { pl: 'Podział contextu', en: 'Context splitting' },
          def: {
            pl: 'Rozbicie jednego dużego contextu na rzadko zmienne <em>akcje</em> i często zmienny <em>stan</em>. Komponent wołający tylko <code>logout</code> przestaje się wtedy renderować przy każdej zmianie danych.',
        ...
        {
          term: { pl: 'Domyślna wartość contextu', en: 'Context default value' },
          def: {
            pl: 'Cicha pułapka: komponent użyty poza providerem dostaje ją bez ostrzeżenia. Przekaż <code>null</code> i opakuj odczyt w hooka, który rzuca błędem.',
        ...
        },
        {
          ...
          def: {
            pl: 'Context czyta się w górę od miejsca, w którym element powstał. Gdy komponent przyszedł przez <code>children</code>, providerem jest ten, kto ten element stworzył.',
      ...
      diagram: {
        svg: '<svg viewBox="0 0 640 420" xmlns="http://www.w3.org/2000/svg" font-family="inherit">' +
          ...
          '</svg>',
        caption: {
          pl: 'Oba mechanizmy omijają props drilling, ale Vue przekazuje reaktywną referencję, a React nową wartość, która budzi wszystkich konsumentów.',
      ...
      levels: {
        eli5: {
          pl: '<p>Wyobraź sobie stary dom, w którym na parterze stoi radio. Radio gra na cały dom. Nikt nie musi biegać po schodach i powtarzać każdemu piętru, jaka piosenka leci. Wystarczy, że ktokolwiek chce posłuchać, otworzy drzwi pokoju.</p><p>W aplikacji jest podobnie. Czasem coś dotyczy całego domu: motyw jasny albo ciemny, zalogowany użytkownik, język. Podawanie tego pokój po pokoju byłoby męczące, bo środkowe piętra wcale tego nie potrzebują - tylko przekazują dalej.</p><p>Więc zamiast tego włączamy radio. Ktoś na górze mówi: <strong>od teraz gra ciemny motyw</strong>. Każdy pokój niżej może tego posłuchać bez pytania sąsiada.</p><p>Vue i React mają obie takie radio, tylko inaczej nazwane. I jest jeden haczyk: w Reactie, gdy zmienisz piosenkę, budzi się każdy pokój, który słuchał - nawet jeśli chciał tylko głośność. Trzeba to robić z głową.</p>',
        ...
        school: {
          pl: '<p>Problem jest ten sam w obu bibliotekach: props drilling, czyli przepychanie wartości przez pięć komponentów, z których cztery jej nie używają.</p><pre><code>// Vue: provide / inject\n// Theme.vue (rodzic)\nconst theme = ref("dark");\nprovide("theme", theme);\n\n// Button.vue (gdziekolwiek niżej)\nconst theme = inject("theme");\n// theme.value działa reaktywnie</code></pre><pre><code>// React: Context\nconst ThemeContext = createContext("light");\n\nfunction App() {\n  const [theme, setTheme] = useState("dark");\n  return (\n    &lt;ThemeContext value={theme}&gt;\n      &lt;Page /&gt;\n    &lt;/ThemeContext&gt;\n  );\n}\n\nfunction Button() {\n  const theme = useContext(ThemeContext);\n}</code></pre><p><strong>We Vue wstrzykiwałeś reaktywną referencję, w Reactie przekazujesz zwykłą wartość, bo React nie ma reaktywności na poziomie obiektu - ma tylko rendery.</strong> Konsekwencja: we Vue zmiana <code>theme.value</code> odświeża dokładnie te komponenty, które czytają tę referencję. W Reactie nowa wartość contextu powoduje ponowny render <em>wszystkich</em> konsumentów, nawet jeśli czytają tylko jedno pole obiektu.</p><p>Druga różnica to klucze. Vue używa stringa lub <code>InjectionKey</code>, więc łatwo o literówkę i o brak typu. React używa obiektu contextu, więc pomyłka jest niemożliwa, a typ wynika z <code>createContext&lt;Theme&gt;()</code>.</p><p>Trzecia: w React 19 <code>&lt;ThemeContext&gt;</code> jest samo w sobie providerem, <code>.Provider</code> nie jest już potrzebny, a nowy hook <code>use(ThemeContext)</code> można wywołać warunkowo, czego <code>useContext</code> nie pozwalał.</p><p>Zasada praktyczna, ta sama co we Vue: context służy do rzeczy rzadko zmiennych i szeroko potrzebnych. Do stanu, który zmienia się co kliknięcie, użyj Zustand albo TanStack Query - dokładnie tak, jak we Vue sięgasz po Pinię zamiast po provide.</p>',
        ...
        pro: {
          pl: '<p>Wydajność jest tu jedyna prawdziwa różnica architektoniczna. Vue subskrybuje efekt renderujący do konkretnego <code>ref</code>. React nie subskrybuje niczego - propaguje zmianę po drzewie i budzi każdy komponent, który zawołał <code>useContext</code> dla tego contextu. <code>React.memo</code> po drodze tego nie zatrzyma.</p><pre><code>// ANTYWZORZEC: nowy obiekt przy każdym renderze\n&lt;AuthContext value={{ user, login, logout }}&gt;\n\n// LEPIEJ: stabilna referencja\nconst value = useMemo(\n  () =&gt; ({ user, login, logout }),\n  [user, login, logout]\n);\n&lt;AuthContext value={value}&gt;</code></pre><p>Standardowy chwyt na duże konteksty to rozbicie na dwa: osobno rzadko zmienne <em>akcje</em> (stabilne na zawsze), osobno często zmienny <em>stan</em>. Komponent, który tylko wywołuje <code>logout</code>, nie przerenderuje się przy każdej zmianie usera.</p><pre><code>const StateCtx = createContext(null);\nconst ActionsCtx = createContext(null);\n\nfunction Provider({ children }) {\n  const [state, dispatch] = useReducer(reducer, init);\n  const actions = useMemo(() =&gt; ({\n    login: (u) =&gt; dispatch({ type: "login", u }),\n    logout: () =&gt; dispatch({ type: "logout" }),\n  }), []);\n  return (\n    &lt;ActionsCtx value={actions}&gt;\n      &lt;StateCtx value={state}&gt;{children}&lt;/StateCtx&gt;\n    &lt;/ActionsCtx&gt;\n  );\n}</code></pre><p>Co jeszcze warto wiedzieć:</p><ul><li>Domyślna wartość z <code>createContext</code> jest cicha pułapka - komponent użyty poza providerem dostanie ją bez ostrzeżenia. Daj <code>null</code> i własny hook, który rzuca błąd. To odpowiednik <code>inject(key)</code> bez wartości domyślnej.</li><li>Context czyta się w górę od miejsca renderu, nie od miejsca w DOM. Jeśli komponent trafił tam przez <code>children</code>, providerem jest ten, który element <em>stworzył</em>, dokładnie jak w Vue.</li><li>Zagnieżdżone providery nadpisują się po poddrzewie - to samo zachowanie co przy <code>provide</code> w Vue i to jest kanoniczna technika w compound components.</li><li>W RSC (React Server Components) context nie działa w komponentach serwerowych. Provider musi być plikiem z <code>use client</code>. We Vue z Nuxtem tego ograniczenia nie ma.</li><li>Gdy dane są duże i selektywnie czytane, użyj <code>useSyncExternalStore</code> albo od razu Zustand. Zustand jest tu odpowiednikiem Pinii, w tym z selektorami, których Context nie ma.</li></ul><p>Pytanie rekrutacyjne: dlaczego Context nie zastępuje Reduxa. Odpowiedź: Context jest mechanizmem dystrybucji, nie sklepem stanu - nie ma selektorów, więc każda zmiana wartości to render wszystkich konsumentów.</p>',
      ...
      quiz: [
        {
          q: {
            pl: 'Która para odpowiada sobie najlepiej?',
          ...
          correct: 0,
          explain: {
            pl: 'Oba rozwiązują ten sam problem: udostępnienie wartości całemu poddrzewu z pominięciem pośrednich komponentów.',
        ...
        },
        {
          ...
          options: [
            { pl: 'React zabrania obiektów w contextcie', en: 'React forbids objects in context' },
            { pl: 'Każdy render rodzica tworzy nową referencję, więc wszyscy konsumenci renderują się ponownie', en: 'Every parent render creates a new reference, so all consumers re-render' },
            { pl: 'Obiekt nie przechodzi przez granicę RSC', en: 'The object cannot cross the RSC boundary' },
            { pl: 'Powstaje wyciek pamięci', en: 'It leaks memory' }
          ],
          correct: 1,
          explain: {
            pl: 'React porównuje wartość contextu przez Object.is. Nowy obiekt to nowa wartość, więc budzi każdego konsumenta. Ratunkiem jest useMemo albo rozbicie contextu.',
        ...
        {
          q: {
            pl: 'We Vue inject zwraca ref i tylko komponenty czytające go się odświeżają. Co jest najbliższa odpowiednia strategia w Reactie?',
            en: 'In Vue inject returns a ref and only the components reading it refresh. What is the closest React strategy?'
          },
          options: [
            { pl: 'Owinąć konsumentów w React.memo', en: 'Wrap consumers in React.memo' },
            { pl: 'Użyć useRef zamiast useState w providerze', en: 'Use useRef instead of useState in the provider' },
            { pl: 'Rozbić context na kilka mniejszych albo użyć sklepu z selektorami, np. Zustand', en: 'Split the context into smaller ones, or use a store with selectors such as Zustand' },
            { pl: 'Wywoływać useContext warunkowo', en: 'Call useContext conditionally' }
          ],
          correct: 2,
          explain: {
            pl: 'Context nie ma selektorów, więc granularność uzyskujesz dzieląc go na mniejsze konteksty albo wychodząc do sklepu z subskrypcją per selektor. memo nie zatrzymuje propagacji contextu.',
        ...
        {
          q: {
            pl: 'Komponent użyty poza providerem czyta context stworzony przez createContext("light"). Co się stanie?',
            en: 'A component used outside the provider reads a context created with createContext("light"). What happens?'
          },
          options: [
            { pl: 'React rzuci błąd w trybie deweloperskim', en: 'React throws in development mode' },
            { pl: 'Dostanie undefined i komponent się wywali', en: 'It receives undefined and the component crashes' },
            { pl: 'Renderowanie zostanie zawieszone do czasu pojawienia się providera', en: 'Rendering suspends until a provider appears' },
            { pl: 'Cicho dostanie wartość domyślną "light" i błąd wyjdzie dopiero na produkcji', en: 'It silently gets the default "light" and the bug surfaces only in production' }
          ],
          correct: 3,
          explain: {
            pl: 'Wartość domyślna maskuje brak providera. Dlatego w produkcyjnym kodzie daje się null i własny hook rzucający czytelny błąd, podobnie jak inject bez wartości domyślnej we Vue.',
    ...
    // ------------------------------------------------------------------ 3
    {
      ...
      terms: [
        {
          ...
          def: {
            pl: 'Pole, dla którego jedynym źródłem prawdy jest stan Reacta, a DOM jest tylko widokiem. Włącza je props <code>value</code> plus <code>onChange</code>.',
        ...
        },
        {
          ...
          def: {
            pl: 'Pole, w którym prawda żyje w DOM, a wartość czytasz refem albo z <code>FormData</code>. Włącza je <code>defaultValue</code> i kosztuje zero renderów na znak.',
        ...
        {
          term: { pl: 'Przełączenie controlled na uncontrolled', en: 'Controlled to uncontrolled switch' },
          def: {
            pl: 'Zmiana <code>value</code> z <code>undefined</code> na string w trakcie życia pola daje ostrzeżenie Reacta i realny błąd. Pisz <code>value={data?.name ?? ""}</code>.',
        ...
        },
        {
          ...
          def: {
            pl: 'W Reactie <code>onChange</code> odpala się przy każdym znaku, a nie na blur jak natywne <code>change</code>. Odpowiednikiem <code>v-model.lazy</code> jest <code>onBlur</code>.',
        ...
        },
        {
          ...
          def: {
            pl: 'Produkcyjna odpowiedź na koszt kontrolowanych formularzy: pola zostają niekontrolowane, a subskrypcja idzie per pole. Odkupuje granularność, którą Vue daje z automatu.',
      ...
      diagram: {
        svg: '<svg viewBox="0 0 640 400" xmlns="http://www.w3.org/2000/svg" font-family="inherit">' +
          ...
          '</svg>',
        caption: {
          pl: 'v-model to skrót na dwie rzeczy naraz; React każe napisać obie połowy pętli, dzięki czemu widać, że źródłem prawdy jest stan.',
      ...
      interactive: {
        kind: 'frames',
        caption: {
          pl: 'Jeden naciśnięty klawisz przechodzi przez całą pętlę kontrolowanego inputa - od zdarzenia do nowej wartości w DOM.',
        ...
        frames: [
          {
            ...
            note: {
              pl: 'Stan trzyma napis An, a input tylko go odbija. Pętla czeka na zdarzenie.',
          ...
          },
          {
            svg: '<svg viewBox="0 0 640 400" xmlns="http://www.w3.org/2000/svg" font-family="inherit">' +
              ...
              '</svg>',
            label: { pl: 'Użytkownik pisze', en: 'User types' },
            note: {
              pl: 'DOM chwilowo pokazuje Ana, ale stan wciąż ma An. Zdarzenie rusza w górę pętli.',
          ...
          },
          {
            svg: '<svg viewBox="0 0 640 400" xmlns="http://www.w3.org/2000/svg" font-family="inherit">' +
              ...
              '</svg>',
            label: { pl: 'Handler czyta wartość', en: 'Handler reads the value' },
            note: {
              pl: 'onChange dostaje e.target.value, czyli Ana. To jedyne miejsce, w którym React dowiaduje się o zmianie.',
          ...
          },
          {
            ...
            note: {
              pl: 'setName zapisuje nową wartość i planuje render. Nic nie dzieje się synchronicznie - zmienna name w tym wywołaniu wciąż ma starą wartość.',
          ...
          },
          {
            svg: '<svg viewBox="0 0 640 400" xmlns="http://www.w3.org/2000/svg" font-family="inherit">' +
              ...
              '</svg>',
            label: { pl: 'Render domyka pętlę', en: 'Render closes the loop' },
            note: {
              pl: 'Komponent renderuje się ponownie i wpisuje value z nowego stanu. Gdyby setName nie zostało wywołane, input cofnąłby się do An i wyglądałby na zablokowany.',
      ...
      levels: {
        eli5: {
          pl: '<p>Wyobraź sobie tablicę w kuchni, na której zapisujesz listę zakupów. Są dwa sposoby pracy z taką tablicą.</p><p>Pierwszy: każdy pisze na niej bezpośrednio kredą. Szybko, ale nikt nie wie, co tam właściwie jest, dopóki nie podejdzie i nie przeczyta.</p><p>Drugi: prawdziwa lista leży w zeszycie, a tablica tylko ją przepisuje. Chcesz coś dopisać? Mówisz o tym, ktoś poprawia zeszyt, a tablica sama się odświeża. Zeszyt jest zawsze prawdą.</p><p>React wybiera drugi sposób. Pole tekstowe nic nie pamięta samo z siebie - pokazuje to, co jest w zeszycie, czyli w stanie. Każde naciśnięcie klawisza to zgłoszenie: <strong>zapisz to proszę w zeszycie</strong>.</p><p>Vue robi to samo, tylko chowa jedną linijkę pod nazwą v-model. Efekt taki sam, różnica jest taka, że w Reactie widzisz obie połówki na własne oczy. Jeśli zapomnisz o połówce, która zapisuje zeszyt, tablica będzie wyglądać na zepsutą.</p>',
        ...
        school: {
          pl: '<p>We Vue piszesz jedną linijkę i temat jest zamknięty:</p><pre><code>&lt;!-- Vue --&gt;\n&lt;script setup&gt;\nconst name = ref("");\n&lt;/script&gt;\n&lt;input v-model="name" /&gt;</code></pre><p>W Reactie piszesz obie połowy:</p><pre><code>// React\nfunction NameField() {\n  const [name, setName] = useState("");\n  return (\n    &lt;input\n      value={name}\n      onChange={(e) =&gt; setName(e.target.value)}\n    /&gt;\n  );\n}</code></pre><p><strong>We Vue używałeś v-model, w Reactie łączysz value i onChange, bo React nie ma dwukierunkowego wiązania - ma jednokierunkowy przepływ danych i zdarzenia wracające w górę.</strong> To nie jest gorzej, tylko jawniej: v-model też rozwija się do <code>:value</code> plus <code>@input</code>, tylko robi to za ciebie kompilator.</p><p>Ta jawność od razu się opłaca, bo transformacje wpisujesz w miejscu, gdzie i tak jesteś:</p><pre><code>onChange={(e) =&gt; setName(e.target.value.toUpperCase())}</code></pre><p>We Vue potrzebowałbyś do tego modyfikatora, computed z setterem albo watcha.</p><p>Ważna pułapka: jeśli podasz <code>value</code> bez <code>onChange</code>, React zablokuje pole i wypisze ostrzeżenie. To odpowiednik <code>:value="name"</code> bez nasłuchu we Vue - tam też nic by się nie wpisało.</p><p>Własny komponent też różni się tylko konwencją nazw. Vue oczekuje propsa <code>modelValue</code> i zdarzenia <code>update:modelValue</code> (albo makra <code>defineModel</code>). React oczekuje pary <code>value</code> i <code>onChange</code>, którą sam ustalasz - to zwykle propsy, więc komponent może mieć ich kilka: <code>value</code>, <code>onChange</code>, <code>selected</code>, <code>onSelect</code>. We Vue byłoby to <code>v-model:selected</code>.</p>',
        ...
        pro: {
          pl: '<p>Kontrolowany kontra niekontrolowany to decyzja architektoniczna, nie stylistyczna. Kontrolowany znaczy: stan Reacta jest jedynym źródłem prawdy, DOM jest widokiem. Niekontrolowany znaczy: prawda siedzi w DOM, a ty czytasz ją refem albo przez FormData.</p><pre><code>// niekontrolowany - zero renderów na klawisz\nfunction Search() {\n  const ref = useRef(null);\n  return (\n    &lt;form action={(fd) =&gt; find(fd.get("q"))}&gt;\n      &lt;input name="q" defaultValue="" ref={ref} /&gt;\n    &lt;/form&gt;\n  );\n}</code></pre><p>Klucz: <code>value</code> robi input kontrolowanym, <code>defaultValue</code> niekontrolowanym. Przejście z <code>undefined</code> na string w trakcie życia komponentu daje ostrzeżenie React i realny bug - typowo, gdy dane przychodzą z API. Używaj <code>value={data?.name ?? ""}</code>.</p><p>Własny komponent z parą value/onChange:</p><pre><code>// React\nfunction Money({ value, onChange }) {\n  return &lt;input value={value} onChange={(e) =&gt; onChange(clamp(e.target.value))} /&gt;;\n}\n\n// Vue: to samo przez defineModel\n// const model = defineModel();\n// &lt;input :value="model" @input="model = clamp($event.target.value)" /&gt;</code></pre><p>Rzeczy, na których ludzie się przewracają:</p><ul><li><strong>Kursor skacze na koniec.</strong> Jeśli w onChange formatujesz tekst (np. wstawiasz spację w numerze karty), musisz ręcznie przywrócić <code>selectionStart</code> w <code>useLayoutEffect</code>. Vue ma ten sam problem, tylko rzadziej się na niego trafia, bo formatuje się w <code>@blur</code>.</li><li><strong>IME i języki azjatyckie.</strong> Kontrolowany input plus agresywne formatowanie psuje składanie znaków. Reaguj na <code>compositionstart</code> i <code>compositionend</code>; we Vue robi to za ciebie <code>v-model</code>.</li><li><strong>Wydajność.</strong> Każdy klawisz to render całego komponentu z formularzem. Przy 30 polach w jednym komponencie widać to na słabszych telefonach. Odpowiedź produkcyjna: react-hook-form, który trzyma pola niekontrolowane i subskrybuje się punktowo - to jest reactowy sposób odzyskania granularności, którą we Vue masz z pudełka.</li><li><strong>onChange w Reactie to tak naprawdę input.</strong> Odpala się przy każdym znaku, nie przy blurze jak natywne zdarzenie change. Odpowiednik <code>v-model.lazy</code> to <code>onBlur</code>.</li><li><strong>Debounce.</strong> Nie debouncuj <code>value</code>, bo pole zacznie gubić znaki. Debouncuj efekt uboczny albo użyj <code>useDeferredValue</code> dla ciężkiej listy wyników.</li></ul><p>Pytanie rekrutacyjne: kiedy input niekontrolowany. Odpowiedź: gdy nie potrzebujesz wartości przy każdym znaku - proste formularze, upload plików, integracje z bibliotekami, które same piszą po DOM.</p>',
      ...
      quiz: [
        {
          q: {
            pl: 'Do czego rozwija się v-model na natywnym inpucie?',
            en: 'What does v-model on a native input expand to?'
          },
          options: [
            { pl: 'Do dwukierunkowego wiązania na poziomie DOM, którego React nie ma', en: 'To DOM-level two-way binding that React lacks' },
            { pl: 'Do bindowania value oraz nasłuchu zdarzenia, czyli dokładnie tego, co w Reactie piszesz ręcznie', en: 'To a value binding plus an event listener, exactly what you write by hand in React' },
            { pl: 'Do watchera na refie', en: 'To a watcher on the ref' },
            { pl: 'Do provide/inject', en: 'To provide/inject' }
          ],
          correct: 1,
          explain: {
            pl: 'To cukier składniowy kompilatora. React nie ma tego cukru, więc obie połowy są widoczne w kodzie.',
            en: 'It is compiler sugar. React has no such sugar, so both halves stay visible in your code.'
          }
        },
        {
          q: {
            pl: 'Input dostaje value bez onChange. Co zobaczy użytkownik?',
            en: 'An input gets value without onChange. What does the user see?'
          },
          options: [
            { pl: 'Pole działa normalnie, React sam aktualizuje stan', en: 'The field works normally, React updates the state itself' },
            { pl: 'Pole nie przyjmuje znaków, a React ostrzega w konsoli', en: 'The field accepts no characters and React warns in the console' },
            { pl: 'Aplikacja rzuca wyjątkiem przy montowaniu', en: 'The app throws on mount' },
            { pl: 'Pole staje się niekontrolowane', en: 'The field becomes uncontrolled' }
          ],
          correct: 1,
          explain: {
            pl: 'Po każdym znaku React przywraca wartość ze stanu, który się nie zmienił, więc pole wygląda na zamrożone. Do odczytu bez edycji służy readOnly.',
        ...
        {
          q: {
            pl: 'Formularz z 30 polami w jednym komponencie muli na telefonie. Jakie rozwiązanie jest reactowo poprawne?',
            en: 'A 30-field form in a single component lags on mobile. Which fix is idiomatic React?'
          },
          options: [
            { pl: 'Debounce na propsie value każdego pola', en: 'Debounce the value prop of every field' },
            { pl: 'Zamienić useState na useRef i renderować ręcznie', en: 'Swap useState for useRef and render manually' },
            { pl: 'Owinąć formularz w Context', en: 'Wrap the form in a Context' },
            { pl: 'Użyć react-hook-form z polami niekontrolowanymi i subskrypcją per pole', en: 'Use react-hook-form with uncontrolled fields and per-field subscriptions' }
          ],
          correct: 3,
          explain: {
            pl: 'Zamiast renderować cały formularz przy każdym znaku, react-hook-form trzyma prawdę w DOM i budzi tylko zainteresowane pola. Debounce na value gubi znaki.',
        ...
        {
          q: {
            pl: 'Pole edycji dostaje value={user?.name}, a user przychodzi z API po sekundzie. Na czym polega błąd?',
            en: 'An edit field gets value={user?.name} and user arrives from the API a second later. What is the bug?'
          },
          options: [
            { pl: 'Input startuje jako niekontrolowany (undefined) i zmienia się w kontrolowany, co React zgłasza jako ostrzeżenie i co gubi wpisany tekst', en: 'The input starts uncontrolled (undefined) and becomes controlled, which React warns about and which loses typed text' },
            { pl: 'Nic się nie stanie, React obsługuje undefined poprawnie', en: 'Nothing happens, React handles undefined fine' },
            { pl: 'Trzeba dodać key, żeby wymusić remount', en: 'You must add a key to force a remount' },
            { pl: 'Błąd wystąpi tylko w StrictMode', en: 'The bug only appears in StrictMode' }
          ],
          correct: 0,
          explain: {
            pl: 'undefined oznacza input niekontrolowany. Zawsze podawaj wartość domyślną, na przykład value={user?.name ?? ""}, albo montuj formularz dopiero po załadowaniu danych.',
    ...
    // ------------------------------------------------------------------ 4
    {
      ...
      terms: [
        {
          ...
          def: {
            pl: 'Rodzina powiązanych komponentów dzieląca stan przez wewnętrzny Context, np. <code>Tabs</code>, <code>Tabs.Trigger</code>, <code>Tabs.Panel</code>. Odpowiednik <code>provide</code> z <code>InjectionKey</code> we Vue.',
        ...
        },
        {
          ...
          def: {
            pl: 'Props będący funkcją, którą rodzic woła z danymi i która zwraca JSX. Odpowiednik scoped slota - dzieli logikę razem z kontrolą nad kawałkiem drzewa.',
        ...
        },
        {
          ...
          def: {
            pl: 'Wariant render propa, w którym funkcja idzie przez <code>children</code>: <code>&lt;List&gt;{(item) =&gt; ...}&lt;/List&gt;</code>. Kosztuje nową funkcję na każdy render, więc psuje <code>React.memo</code>.',
        ...
        },
        {
          ...
          def: {
            pl: 'Biblioteka dająca zachowanie, stan i dostępność bez własnych stylów ani sztywnej struktury DOM. Dziś domyślnie budowana właśnie na compound components.',
        ...
        {
          term: { pl: 'Hook strażnik', en: 'Guard hook' },
          def: {
            pl: 'Hook typu <code>useTabs()</code>, który rzuca błędem, gdy część rodziny została użyta poza rodzicem. Zastępuje kruche sprawdzanie typów dzieci przez <code>Children.map</code>.',
      ...
      diagram: {
        svg: '<svg viewBox="0 0 640 420" xmlns="http://www.w3.org/2000/svg" font-family="inherit">' +
          ...
          '</svg>',
        caption: {
          pl: 'Scoped slot i render prop robią to samo: oddają dane do rodzica. Compound components idą krok dalej i dzielą stan przez Context.',
      ...
      levels: {
        eli5: {
          pl: '<p>Wyobraź sobie ekspres do kawy w biurze. Można go zbudować na dwa sposoby.</p><p>Pierwszy: jeden wielki przycisk i dwadzieścia pokręteł z tyłu. Każdy nowy pomysł to nowe pokrętło. Po roku nikt nie wie, co robi pokrętło numer siedemnaście.</p><p>Drugi: zestaw części, które do siebie pasują - młynek, zaparzacz, spieniacz. Ustawiasz je w kolejności, która ci pasuje, a one same gadają między sobą i wiedzą, że należą do jednego ekspresu.</p><p>Drugi sposób to <strong>compound components</strong>, czyli komponenty składane. Zamiast dwudziestu opcji dajesz klocki.</p><p>Jest też druga sztuczka. Czasem klocek wie coś, czego ty nie wiesz - na przykład która zakładka jest otwarta. Wtedy klocek podaje ci tę informację, a ty decydujesz, jak ją narysować. To jakbyś powiedział ekspresowi: <em>ty pilnuj temperatury, ja wybieram kubek</em>.</p>',
        ...
        school: {
          pl: '<p>Znasz ten problem z design systemu: komponent zaczyna od trzech propsów, a po roku ma <code>showIcon</code>, <code>iconPosition</code>, <code>headerVariant</code> i <code>dense</code>. To jest konfiguracja zamiast kompozycji.</p><p>We Vue ratunkiem były sloty, zwłaszcza scoped:</p><pre><code>&lt;!-- Vue --&gt;\n&lt;DataList :items="rows"&gt;\n  &lt;template #row="{ item, selected }"&gt;\n    &lt;b :class="{ on: selected }"&gt;{{ item.name }}&lt;/b&gt;\n  &lt;/template&gt;\n&lt;/DataList&gt;</code></pre><p>W Reactie odpowiednikiem scoped slota jest <strong>render prop</strong>: props, który jest funkcją i dostaje dane od dziecka.</p><pre><code>// React\n&lt;DataList items={rows}&gt;\n  {({ item, selected }) =&gt; (\n    &lt;b className={selected ? "on" : ""}&gt;{item.name}&lt;/b&gt;\n  )}\n&lt;/DataList&gt;</code></pre><p><strong>We Vue oddawałeś dane przez v-slot z parametrem, w Reactie przekazujesz funkcję jako children, bo children to zwykła wartość, a funkcja to też wartość.</strong> Żadnego nowego API - dziecko po prostu wywołuje <code>children(dane)</code>.</p><p>Druga technika to compound components. Komponent nadrzędny trzyma stan i wystawia go przez Context, a części składowe go czytają:</p><pre><code>&lt;Tabs defaultValue="a"&gt;\n  &lt;Tabs.List&gt;\n    &lt;Tabs.Trigger value="a"&gt;Ogólne&lt;/Tabs.Trigger&gt;\n    &lt;Tabs.Trigger value="b"&gt;Zaawansowane&lt;/Tabs.Trigger&gt;\n  &lt;/Tabs.List&gt;\n  &lt;Tabs.Panel value="a"&gt;...&lt;/Tabs.Panel&gt;\n&lt;/Tabs&gt;</code></pre><p>Użytkownik ustawia elementy tam, gdzie chce, dokładać własne opakowania i klasy, a stan aktywnej zakładki pozostaje prywatny. To ta sama idea, którą we Vue realizujesz przez <code>provide</code> w rodzicu i <code>inject</code> w częściach - Radix UI i Headless UI zbudowały na tym całe biblioteki.</p><p>Zasada wyboru jest prosta i taka sama jak we Vue: jeśli komponent dostaje kolejnego propsa typu boolean, który tylko włącza inny kawałek widoku, prawdopodobnie powinien być to osobny element składowy albo dziura na treść.</p>',
        ...
        pro: {
          pl: '<p>Compound components są dziś domyślnym wzorcem dla headless UI, bo rozwiązują trzy problemy naraz: eksplozję propsów, sztywną strukturę DOM i dostępność, którą chcesz mieć w bibliotece, a nie w każdej aplikacji.</p><pre><code>const TabsCtx = createContext(null);\n\nfunction Tabs({ defaultValue, children }) {\n  const [value, setValue] = useState(defaultValue);\n  const ctx = useMemo(() =&gt; ({ value, setValue }), [value]);\n  return &lt;TabsCtx value={ctx}&gt;{children}&lt;/TabsCtx&gt;;\n}\n\nfunction useTabs() {\n  const ctx = useContext(TabsCtx);\n  if (!ctx) throw new Error("Tabs.* must be used inside &lt;Tabs&gt;");\n  return ctx;\n}\n\nTabs.Trigger = function Trigger({ value, children }) {\n  const t = useTabs();\n  return (\n    &lt;button role="tab" aria-selected={t.value === value}\n      onClick={() =&gt; t.setValue(value)}&gt;{children}&lt;/button&gt;\n  );\n};</code></pre><p>Vue rozwiązuje to samo przez <code>provide</code> plus <code>InjectionKey</code>, tylko części są osobnymi plikami SFC zamiast właściwości obiektu. Kropka w <code>Tabs.Trigger</code> nie ma żadnej magii - to zwykła właściwość funkcji, która poprawia czytelność importów i podpowiedzi w edytorze.</p><p>Render props kontra hooki. Historycznie render props były w Reactie sposobem na współdzielenie logiki; hooki przejęły większość tych zastosowań. Dziś reguły są proste:</p><ul><li>Logika bez własnego JSX - <strong>custom hook</strong>. To odpowiednik composable we Vue i domyślny wybór.</li><li>Dziecko renderuje strukturę, a wołający ma wypełnić jej kawałek - <strong>render prop</strong> albo <code>children</code> jako funkcja. Klasyka: wirtualizowane listy (<code>react-window</code>), <code>Autocomplete</code>, biblioteki wykresów.</li><li>Zestaw powiązanych elementów o wspólnym stanie - <strong>compound components</strong>.</li></ul><p>Koszty, o których trzeba wiedzieć:</p><ul><li>Render prop tworzy nową funkcję przy każdym renderze, więc <code>React.memo</code> na dziecku przestaje działać. Przy dużych listach to widać w profilerze.</li><li>Zagnieżdżanie render props daje piramidę wcięć - to właśnie boli w starym kodzie i to właśnie naprawiły hooki.</li><li>Compound components z Contextem kosztują jeden render całego poddrzewa przy każdej zmianie wartości; przy zakładkach to nieistotne, przy tabeli z zaznaczaniem wierszy już nie - wtedy dziel context albo trzymaj zaznaczenie w sklepie z selektorami.</li><li>Nie waliduj typów dzieci przez <code>Children.map</code> i sprawdzanie <code>child.type</code>. Złamie się przy owinięciu w <code>&lt;div&gt;</code> albo w <code>&lt;Fragment&gt;</code>. Błąd z <code>useTabs</code> wystarczy.</li></ul><p>Pytanie na rozmowie: czym różni się render prop od custom hooka. Odpowiedź: hook dzieli logikę, render prop dzieli logikę razem z kontrolą nad fragmentem drzewa; jeśli nie potrzebujesz tego drugiego, hook jest prostszy i tańszy.</p>',
      ...
      quiz: [
        {
          ...
          correct: 1,
          explain: {
            pl: 'Scoped slot to funkcja, którą dziecko wywołuje z danymi. W Reactie ta funkcja to po prostu props - najczęściej children.',
            en: 'A scoped slot is a function the child calls with data. In React that function is simply a prop, most often children.'
          }
        },
        {
          q: {
            pl: 'Jak Tabs.Panel dowiaduje się, która zakładka jest aktywna?',
            en: 'How does Tabs.Panel learn which tab is active?'
          },
          options: [
            { pl: 'Z Contextu wystawionego przez Tabs, dokładnie jak inject z provide we Vue', en: 'From a Context provided by Tabs, exactly like inject from provide in Vue' },
            { pl: 'Rodzic klonuje dzieci i wstrzykuje im props', en: 'The parent clones its children and injects the prop' },
            { pl: 'Ze zmiennej globalnej modułu', en: 'From a module-level global' },
            { pl: 'Z atrybutu data w DOM', en: 'From a data attribute in the DOM' }
          ],
          correct: 0,
          explain: {
            pl: 'Context pozwala użytkownikowi dowolnie owijać części i przestawiać je w strukturze. Klonowanie dzieci załamuje się przy pierwszym opakowaniu w div.',
        ...
        {
          q: {
            pl: 'Kiedy custom hook jest lepszy niż render prop?',
            en: 'When is a custom hook better than a render prop?'
          },
          options: [
            { pl: 'Zawsze, render props zostały usunięte w React 19', en: 'Always, render props were removed in React 19' },
            { pl: 'Nigdy, hooki nie potrafią dzielić logiki', en: 'Never, hooks cannot share logic' },
            { pl: 'Gdy dzielisz samą logikę i nie musisz kontrolować fragmentu drzewa renderowanego przez dziecko', en: 'When you share logic only and do not need control over a slice of the tree the child renders' },
            { pl: 'Gdy potrzebujesz dostępu do DOM', en: 'When you need DOM access' }
          ],
          correct: 2,
          explain: {
            pl: 'Hook zwraca dane i funkcje, a JSX piszesz normalnie. Render prop zostaw tam, gdzie to dziecko decyduje o strukturze, a ty wypełniasz dziurę.',
        ...
        },
        {
          ...
          options: [
            { pl: 'memo nie działa na komponentach funkcyjnych', en: 'memo does not work on function components' },
            { pl: 'Inline funkcja render prop ma nową tożsamość przy każdym renderze, więc porównanie propsów zawsze zawodzi', en: 'The inline render-prop function has a new identity every render, so the props comparison always fails' },
            { pl: 'Wirtualizacja wyłącza memoizację', en: 'Virtualisation disables memoisation' },
            { pl: 'Context zawsze omija memo', en: 'Context always bypasses memo' }
          ],
          correct: 1,
          explain: {
            pl: 'Nowa funkcja to nowy props. Wyciągnij ją poza render albo owiń w useCallback, ewentualnie przekaż zamiast niej komponent wiersza.',
    ...
    // ------------------------------------------------------------------ 5
    {
      ...
      terms: [
        {
          ...
          def: {
            pl: 'Komponent klasowy łapiący błędy renderu swojego poddrzewa i pokazujący fallback. Odpowiednik <code>onErrorCaptured</code>, tyle że musi być osobnym komponentem.',
        ...
        },
        {
          ...
          def: {
            pl: 'Statyczna metoda wołana w czasie renderu, która zamienia złapany błąd w stan granicy. Powód, dla którego error boundary nadal musi być klasą.',
        ...
        },
        {
          ...
          def: {
            pl: 'Granica pokazująca fallback, gdy komponent w środku zawiesił render w oczekiwaniu na dane. W Next.js każda granica to osobny kawałek strumienia HTML.',
        ...
        },
        {
          term: { pl: 'use()', en: 'use()' },
          def: {
            pl: 'Hook Reacta 19 czytający promise albo context; zawiesza render do rozwiązania promisy. Promisa musi być cache-owana, bo <code>use(fetch(...))</code> w ciele komponentu pętli się w nieskończoność.',
        ...
        },
        {
          ...
          def: {
            pl: 'Mechanizm z <code>react-error-boundary</code> pozwalający wyjść z fallbacku - typowo przy zmianie trasy plus wyczyszczeniu cache zapytań. Sam fallback bez drogi powrotnej to za mało.',
      ...
      diagram: {
        svg: '<svg viewBox="0 0 640 420" xmlns="http://www.w3.org/2000/svg" font-family="inherit">' +
          ...
          '</svg>',
        caption: {
          pl: 'Error boundary łapie wyjątki, Suspense łapie oczekiwanie na dane. Obie granice działają na najbliższym przodku w górę drzewa.',
      ...
      interactive: {
        kind: 'frames',
        caption: {
          pl: 'Życie jednego poddrzewa: render, zawieszenie, fallback, dane albo błąd przechwycony przez granicę.',
        ...
        frames: [
          {
            ...
            note: {
              pl: 'Komponent startuje normalnie i sięga po dane, których jeszcze nie ma.',
          ...
          },
          {
            ...
            note: {
              pl: 'Dane jeszcze nie dotarły, więc render zostaje przerwany i szuka najbliższej granicy Suspense w górę drzewa.',
          ...
          },
          {
            ...
            note: {
              pl: 'Użytkownik widzi szkielet. Reszta strony poza granicą działa normalnie - dokładnie jak default i fallback w Suspense Vue.',
          ...
          },
          {
            svg: '<svg viewBox="0 0 640 400" xmlns="http://www.w3.org/2000/svg" font-family="inherit">' +
              ...
              '</svg>',
            label: { pl: 'Dane dotarły', en: 'Data arrives' },
            note: {
              pl: 'Promise się rozwiązał, React renderuje poddrzewo jeszcze raz i podmienia fallback na treść.',
          ...
          },
          {
            svg: '<svg viewBox="0 0 640 400" xmlns="http://www.w3.org/2000/svg" font-family="inherit">' +
              ...
              '</svg>',
            label: { pl: 'Ścieżka błędu', en: 'The error branch' },
            note: {
              pl: 'Gdy promise odrzuci albo render rzuci wyjątkiem, przejmuje to error boundary. Reszta aplikacji żyje dalej.',
      ...
      levels: {
        eli5: {
          pl: '<p>Wyobraź sobie restaurację. Kelner idzie po zamówienie do kuchni. Mogą zdarzyć się dwie rzeczy.</p><p>Pierwsza: danie się jeszcze robi. Wtedy kelner nie stoi w milczeniu przy stoliku - przynosi pieczywo i mówi, że zaraz będzie. Goście wiedzą, że wszystko idzie zgodnie z planem.</p><p>Druga: kucharz przypalił danie. Wtedy też nie zamykamy całej restauracji. Kelner przychodzi, przeprasza i pyta, czy zamówić coś innego. Reszta sali je spokojnie dalej.</p><p>W aplikacji są dokładnie te dwie sytuacje. <strong>Czekamy</strong> na dane albo <strong>coś poszło nie tak</strong>. React ma na to dwa ogrodzenia: jedno pokazuje pieczywo, czyli szkielet strony, drugie łapie przypalone danie i pokazuje uprzejmy komunikat z przyciskiem <em>spróbuj jeszcze raz</em>.</p><p>Najważniejsze jest to, że ogrodzenie otacza tylko jeden stolik. Reszta aplikacji działa dalej.</p>',
        ...
        school: {
          pl: '<p>We Vue znasz oba mechanizmy, tylko pod innymi nazwami.</p><pre><code>&lt;!-- Vue --&gt;\n&lt;Suspense&gt;\n  &lt;template #default&gt;&lt;Profile /&gt;&lt;/template&gt;\n  &lt;template #fallback&gt;&lt;Skeleton /&gt;&lt;/template&gt;\n&lt;/Suspense&gt;\n\n&lt;script setup&gt;\nonErrorCaptured((err) =&gt; { report(err); return false; });\n&lt;/script&gt;</code></pre><p>W Reactie to dwa osobne komponenty:</p><pre><code>// React\n&lt;ErrorBoundary fallback={&lt;Retry /&gt;}&gt;\n  &lt;Suspense fallback={&lt;Skeleton /&gt;}&gt;\n    &lt;Profile /&gt;\n  &lt;/Suspense&gt;\n&lt;/ErrorBoundary&gt;</code></pre><p><strong>We Vue używałeś hooka cyklu życia onErrorCaptured w dowolnym komponencie, w Reactie musisz użyć komponentu granicznego, bo tylko klasa z getDerivedStateFromError potrafi złapać błąd z renderu poddrzewa.</strong> Hooka do tego nie ma i w React 19 nadal nie ma - w praktyce importujesz gotowy <code>react-error-boundary</code> zamiast pisać klasę.</p><p>Podział obowiązków jest czysty:</p><ul><li><strong>Suspense</strong> łapie oczekiwanie. Komponent, który nie ma jeszcze danych, przerywa render, a najbliższy Suspense w górę pokazuje fallback.</li><li><strong>Error boundary</strong> łapie wyjątki rzucone podczas renderu, w metodach cyklu życia i w konstruktorach dzieci.</li></ul><p>Czego error boundary <em>nie</em> łapie: błędów w handlerach zdarzeń (<code>onClick</code>), w kodzie asynchronicznym poza renderem i w samej granicy. To dokładnie ta sama pułapka co w Vue - <code>onErrorCaptured</code> też nie złapie odrzuconego promise z <code>setTimeout</code>.</p><p>Praktyka: jedna granica na trasę i po jednej wokół ryzykownych wysepek - wykresu, widgetu z zewnętrznego API, edytora. Zbyt szeroka granica zamienia mały błąd w biały ekran, zbyt wąska daje interfejs w kropki.</p>',
        ...
        pro: {
          pl: '<p>Suspense w Reactie odpalasz nie przez API danych, tylko przez mechanizm renderu: komponent, który czyta jeszcze nierozwiązany promise, przerywa render. Od React 19 robi to hook <code>use</code>.</p><pre><code>// React 19\nfunction Profile({ userPromise }) {\n  const user = use(userPromise); // suspenduje dopóki nie rozwiązany\n  return &lt;h1&gt;{user.name}&lt;/h1&gt;;\n}\n\n&lt;ErrorBoundary FallbackComponent={Retry}&gt;\n  &lt;Suspense fallback={&lt;Skeleton /&gt;}&gt;\n    &lt;Profile userPromise={fetchUser(id)} /&gt;\n  &lt;/Suspense&gt;\n&lt;/ErrorBoundary&gt;</code></pre><p>Krytyczna zasada: promise musi powstać <em>poza</em> renderem albo być cache-owany. <code>use(fetch(...))</code> wprost w ciele komponentu tworzy nowy promise przy każdej próbie i daje nieskończoną pętlę. Dlatego promise pochodzi z RSC, z TanStack Query albo z własnego cache po kluczu. We Vue tego problemu nie ma, bo <code>async setup</code> odpala się raz.</p><p>Minimalna granica błędu, jeśli nie chcesz zależności:</p><pre><code>class ErrorBoundary extends React.Component {\n  state = { error: null };\n  static getDerivedStateFromError(error) { return { error }; }\n  componentDidCatch(error, info) { logToSentry(error, info.componentStack); }\n  render() {\n    if (this.state.error) return this.props.fallback;\n    return this.props.children;\n  }\n}</code></pre><p>Produkcyjne szczegóły, które odróżniają działającą aplikację od demo:</p><ul><li><strong>Reset.</strong> Sam fallback nie wystarcza - potrzebujesz drogi powrotnej. <code>react-error-boundary</code> daje <code>resetKeys</code> i <code>onReset</code>; typowo resetujesz przy zmianie trasy i czyścisz cache zapytania.</li><li><strong>Aktualizacje bez migotania.</strong> Zmiana danych w już zamontowanym poddrzewie znowu pokazałaby fallback. <code>startTransition</code> albo <code>useDeferredValue</code> utrzymują starą treść, dopóki nowa nie będzie gotowa. To odpowiednik <code>&lt;Suspense timeout&gt;</code>, o którym myśli się we Vue.</li><li><strong>SSR i streaming.</strong> W Next.js każda granica Suspense to osobny kawałek strumienia HTML - to jest mechanizm, dzięki któremu <code>loading.tsx</code> działa. Zła granulacja granic wprost pogarsza LCP.</li><li><strong>Hydration.</strong> Błąd niezgodności przy hydracji złapie granica, ale React i tak przerenderuje klienta od zera - w konsoli zobaczysz ostrzeżenie, które trzeba naprawić u źródła.</li><li><strong>Telemetria.</strong> <code>componentDidCatch</code> dostaje <code>info.componentStack</code>; wysyłaj to do Sentry razem z identyfikatorem trasy. Bez tego stacktrace z zminifikowanego builda jest bezużyteczny.</li><li><strong>Nie łap błędów biznesowych.</strong> Błąd 404 to stan interfejsu, nie wyjątek. Granica jest dla awarii, których nie przewidziałeś.</li></ul><p>Pytanie rekrutacyjne: czemu error boundary musi być klasą. Odpowiedź: bo łapanie błędu wymaga fazy commit i metody statycznej wywoływanej podczas renderu; ekwiwalentu w hookach nie ma i React nie planuje go w najbliższym czasie.</p>',
      ...
      quiz: [
        {
          ...
          options: [
            ...
            { pl: 'Blok try/catch wokół JSX', en: 'A try/catch block around the JSX' },
            { pl: 'Suspense z propsem onError', en: 'Suspense with an onError prop' }
          ],
          correct: 1,
          explain: {
            pl: 'React nie ma hooka do łapania błędów renderu. Granica musi być klasą, dlatego w praktyce używa się biblioteki react-error-boundary.',
        ...
        {
          q: {
            pl: 'Którego błędu error boundary NIE złapie?',
            en: 'Which error will an error boundary NOT catch?'
          },
          options: [
            { pl: 'Wyjątku rzuconego w handlerze onClick', en: 'A throw inside an onClick handler' },
            { pl: 'Wyjątku w ciele komponentu podczas renderu', en: 'A throw in the component body during render' },
            { pl: 'Wyjątku w useMemo podczas renderu', en: 'A throw inside useMemo during render' },
            { pl: 'Wyjątku w komponencie zagnieżdżonym trzy poziomy niżej', en: 'A throw in a component three levels below' }
          ],
          correct: 0,
          explain: {
            pl: 'Handlery zdarzeń działają poza renderem, więc obsłuż je zwykłym try/catch i stanem błędu. To samo ograniczenie ma onErrorCaptured we Vue.',
        ...
        {
          q: {
            pl: 'Dlaczego use(fetch(url)) wywołane wprost w ciele komponentu wpada w nieskończoną pętlę?',
          ...
          options: [
            ...
            { pl: 'Bo use działa tylko w komponentach serwerowych', en: 'Because use only works in server components' },
            { pl: 'Bo Suspense ma limit trzech prób', en: 'Because Suspense has a three-attempt limit' },
            { pl: 'Bo każda próba renderu tworzy nowy promise, który natychmiast znów zawiesza komponent', en: 'Because every render attempt creates a new promise, which immediately suspends the component again' }
          ],
          correct: 3,
          explain: {
            pl: 'Promise musi być stabilny między próbą a próbą: tworzony w RSC, w loaderze albo cache-owany po kluczu, na przykład przez TanStack Query.',
        ...
        {
          q: {
            pl: 'Zmiana filtra powoduje ponowne pobranie danych i cały panel miga szkieletem. Co jest właściwym rozwiązaniem?',
            en: 'Changing a filter refetches data and the whole panel flashes a skeleton. What is the right fix?'
          },
          options: [
            { pl: 'Owinąć aktualizację w startTransition albo użyć useDeferredValue, żeby stara treść została na ekranie', en: 'Wrap the update in startTransition or use useDeferredValue so the old content stays on screen' },
            { pl: 'Usunąć granicę Suspense', en: 'Remove the Suspense boundary' },
            { pl: 'Dodać key do panelu, żeby wymusić remount', en: 'Add a key to the panel to force a remount' },
            { pl: 'Przenieść granicę błędu wyżej', en: 'Move the error boundary higher' }
          ],
          correct: 0,
          explain: {
            pl: 'Aktualizacje w transition nie chowają już widocznej treści za fallbackiem. Fallback zostaje dla pierwszego załadowania, gdzie faktycznie nie ma czego pokazać.',
